...
static void output_OBJ_file(const std::vector<Point>& vertices, const std::vector<std::vector<unsigned long>> &faces,std::string& fname) {
 ...
 void output_txt_file(const std::vector<Point>& vertices, const std::vector<std::vector<unsigned long>>& faces,std::string& fname) {
...
//                            }
       ...
       // ----use DT to create convex hull -> create polyhedron
                Delaunay T;
                std::string fname2="delaunay.obj";
                output_txt_file(cur_vertices,face_v_indices,fname2);
...
bool vertex_exist_check(std::vector<Point>& vertices, Point& vertex) {
    bool flag= false;
    for (auto& v : vertices) {
        if (CGAL::to_double(v.x()) == CGAL::to_double(vertex.x()) &&
            CGAL::to_double(v.y()) == CGAL::to_double(vertex.y()) &&
        CGAL::to_double(v.z()) == CGAL::to_double(vertex.z()))
        {
            flag= true;
        }
    }
...
public:
    ...
    Jshell()
    {}
    Jshell(std::vector<Point> v1,std::vector<std::vector<unsigned long>> f1)
    {
        vertices=v1;
        faces=f1;
    }
...
std::vector<Jshell> store_shell(const Nef_polyhedron& big_nef) {

    std::vector<Jshell> shell_list;
    ...
    CGAL_forall_volumes(current_volume, big_nef) {
        ...
        CGAL_forall_shells_of(current_shell, current_volume) { // iterate each shell
            ...
            if(volume_count == 5)
            {
                ...
                Jshell jshell=Jshell(se.vertices,se.faces);
                shell_list.emplace_back(jshell);
            }
        }
    }
    return shell_list;
...
void write_cityjson(const std::vector<Jshell>& jshell)
{
    int volume_count = 0;
    nlohmann::json j;
    j["type"] = "CityJSON";
    ...
    j["CityObjects"]["Building-1"]=json::object();

    j["CityObjects"]["Building-1"]["attributes"]=json::object();
    j["CityObjects"]["Building-1"]["children"]=json::array({"Building-1-0"});

    // ----test-----
    j["CityObjects"]["Building-1"]["geometry"] = json::array();
    j["CityObjects"]["Building-1"]["type"]="Building";
    ...
    for(auto & se:jshell)
    {
        ...
        if(all_vertices.empty())
            all_vertices=se.vertices;

        else{
            std::vector<Point> temp=se.vertices;
            for(auto & vertex: temp)
                all_vertices.emplace_back(vertex);
        }

        for(auto & all_vertice : all_vertices)
        {
            if (unique_vertices.empty()) unique_vertices.emplace_back(all_vertice);
            else
            {
                bool mark= false;

                for(auto & unique_vertice : unique_vertices)
                {
                    if(CGAL::to_double(all_vertice.x()) == CGAL::to_double(unique_vertice.x())&&CGAL::to_double(all_vertice.y()) == CGAL::to_double(unique_vertice.y()) && CGAL::to_double(all_vertice.z()) == CGAL::to_double(unique_vertice.z()) )
                    {
                        mark= true;
                    }
                }
                if(!mark) {
                    unique_vertices.emplace_back(all_vertice);
                }
            }
        }
        ...
        for(auto & face:face_v_indices)
        {
            for(auto & vertex: face)
            {
                for(unsigned long i = 0; i < unique_vertices.size(); ++i)
                {
                    if(CGAL::to_double(unique_vertices[i].x())==CGAL::to_double(cur_vertices[vertex-1].x()) && CGAL::to_double(unique_vertices[i].y())==CGAL::to_double(cur_vertices[vertex-1].y()) && CGAL::to_double(unique_vertices[i].z())==CGAL::to_double(cur_vertices[vertex-1].z()))
                    {
                        vertex=i+1;
                        break;
                    }
        ...
        for(const auto& vpoint:unique_vertices)
        {
 ...
 int main()
{
    ...
    std::vector<Jshell> shell_list = store_shell(big_nef);
    write_cityjson(shell_list);
